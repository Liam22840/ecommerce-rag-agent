...
对应 spec 3.3「后端服务化」：API、流式接口、配置、首屏延迟与缓存、降级容错、返回结构。意图与路由见 `IntentAndRouting.md`，数据与检索见 `RetrievalAndData.md`，各 Agent 见对应文档。入口在 `server/app.py`。
...
2. `products` / `comparison`（卡片优先）：检索一完成先把卡片发出去，不等正文。payload 同时含 `products`/`items`，商品字段同时含 `price/base_price` 和 `matched_reason/reason`。
3. `cart` / `order` / `plan`：购物车 / 下单 / 多步规划轮还会发对应帧——`cart` 购物车状态，`order` 订单状态（待确认/已提交/已取消，含金额与收货地址），`plan` 多步计划的逐步进度（每步 running/done 都会再发一帧）。
4. `token`（正文）：随后流式输出正文，payload 同时含 `token`/`delta`/`text` 兼容不同前端。
5. `done`：结束标记，附 `session_id`、`retrieval_source`、`warnings`。
...
- **向量预热（流水线并行）**：`ProductRetriever.prewarm_query()` 在意图 LLM 还在读 query 时就用线程池后台算 query 向量并存成 future，检索阶段直接 `await`，冷查询省约 1s。只有当意图 LLM 把 query 改写成不同文本时预热用不上，退回串行速度，绝不会更慢。embedding 缓存多线程读写已加锁。
- **Prompt 压缩**：回答 prompt 约减半（见 `RetrievalAndData.md` 的 product facts 压缩）。
- **两级回答缓存**：精确缓存 `QueryCache`（按归一化 query + top_k，命中 <0.1s，只缓存无上下文的 `product_search`）；意图缓存 `FilterCache`（按解析后的 `SearchFilters` 语义字段，同义不同写法命中同一条，命中跳过 embedding/检索/回答 LLM，约 1.3s）。catalog 静态，只设 LRU 上限不设 TTL。带购物车/对比/最近商品上下文、或 commerce/planner/attachments 的轮次不走缓存。
