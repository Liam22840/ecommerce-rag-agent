...
一句话：这是一个电商导购 Agent。用户用大白话问，它听懂、去商品库里找，然后像真人导购一样推荐、对比、加购、下单。

整套系统就守一条规矩：**让大模型负责「听懂人话」，让代码负责「核对事实和兜底」。** 模型读懂用户想要什么，但价格、库存、订单号这些不能出错的东西，一律由代码从结构化数据里取，不让模型自己编。万一模型挂了，代码还能降级把基本功能撑住，不至于整个崩掉。
...
路由器：这句话想干嘛？
   ...
   （不管走哪条，最后回答和操作都只基于真实数据，绝不编价格、库存、参数）
```
