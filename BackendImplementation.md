...
3. 先由一个专注的 LLM 路由分类器（`classify_route`）判定这一轮的类别：search / comparison / cart / checkout / plan / chitchat。它只拿到 raw query + 少量压缩上下文（上一轮类别 `last_route`、购物车有没有商品、有没有待确认订单、刚不刚对比过），结合上下文推理而不是匹配关键词（详见下文「意图路由与上下文」）。LLM 不可用时退回关键词路由。
4. 按路由分派：chitchat（含不在售品类）直接回友好短句；comparison → 对比决策（见 `ComparisonDecisionAgent.md`）；cart / checkout → 购物车与下单（见 `CartCheckoutAgent.md`）；plan → 多步规划（见 `PlannerAgent.md`）；search → 商品检索。只有 search / comparison 才跑较重的 `IntentParser` 解析筛选条件（确定性规则校验/兜底，并结合 session 历史补全多轮上下文：承接、相对追问、“换一批”、回看之前商品等）。带图片的请求在路由之前就走图搜 `_prepare_photo_search`。
...
1. `token`（开场白）：请求一进来就先发一句中性短开场白（`opener_lead`，如“好的，”），此时路由都还没跑，所以首 token 时间在毫秒级；等路由判定后再补一句和类别相关的尾巴（`opener_continuation`，如“我来帮您找面霜～”“对比一下”，chitchat 没有尾巴、由其内联回复自报）。开场白只是流式装饰，不写入存储或非流式 JSON 回答；它是延迟手段而非路由，猜错只是多/少一句开场白，不改变 LLM 实际选的路由。
...
3. `cart` / `order` / `plan`：购物车 / 下单 / 多步规划轮还会发对应帧——`cart` 承载购物车状态，`order` 承载订单状态（待确认/已提交/已取消，含金额与收货地址），`plan` 承载多步计划的逐步进度（每步 running/done 都会再发一帧）。
4. `token`（正文）：随后才流式输出 LLM 正文。payload 同时包含 `token`、`delta` 和 `text`，兼容不同前端 parser。
5. `done`：流结束标记，并附带 `session_id`、`retrieval_source`、`warnings` 等调试信息。

SSE 的 content type 是 `text/event-stream; charset=utf-8`，并且 stream parser 使用 UTF-8 解码，避免中文流式输出乱码。早期版本曾发送 `meta` 和 `delta` 事件；当前版本发送 `token / products / comparison / cart / order / plan / done`，是为了兼容 Liam 的 iOS 前端和旧构建缓存。若 prepare 在生成器内部抛错（开场白已发出，无法再返回 500），会优雅地补一句兜底文案再 `done`，不会让前端挂住。
...
路由器（`classify_route`，提示 `ROUTER_SYSTEM`）是一个专注、低延迟的分类器——它在毫秒级首 token 路径上跑，所以刻意只拿很少的上下文：raw query + 几个压缩信号（上一轮类别 `last_route`、购物车有没有商品、刚展示过商品没有、有没有待确认订单、刚不刚对比过）。这是经典的 dialogue state tracking：把对话压缩成结构化状态喂给分类器，而不是塞整段历史。

`last_route`（上一轮意图类别，如“处理下单或订单”“找/展示商品”）是补上的那点上下文。没有它时，订单提交后一句空洞的“确认”和凭空说“确认”长得一模一样，路由器只能猜，于是有时猜成 search，搜索再对没有真实诉求的输入返回最近邻商品（看上去就是“随机商品”）。

提示让路由器先判断这句有没有真正的购物诉求（找/浏览商品、操作购物车、下单），再结合 `last_route` 与“有待确认订单”推理：有草稿时的简短同意归 checkout；没有草稿时刚下完单 / 刚闲聊后的附和、致谢、确认归 chitchat；想随意浏览（“随便看看”）仍归 search。这样它能泛化到没列进提示的空洞词（得嘞 / ok啦 / 嗯哼），而不是靠词表。

为什么不用“相关度阈值”兜底搜索：标定约 100 条 query 后发现，向量相似度无法分开真实与垃圾——真实的“冬天穿的外套”(0.32) 比垃圾“好的”(0.41) 还低、比不在售“冰箱”(0.34) 还低，lexical 也不可靠（“好的”命中 12 个商品的单字）。所以分辨它们只能靠理解语义的 LLM 路由，而不是分数阈值。对边界浏览问句（“有什么推荐”），路由器落到一句“您想看哪一类”的澄清——该问就问，而不是猜着回一堆随机商品。
...
- 回答缓存：精确缓存 + 按意图的同义缓存。
- Multimodal query：图搜已上线——用户上传图片（可带 caption），VLM 解析 + 图像向量复用同一 hybrid 管线（`_prepare_photo_search`、`parse_image`、`embed_image`）。
- Cart/actions：购物车与下单已上线（add/remove/改数量/结算/确认/取消/改地址，见 `CartCheckoutAgent.md`），多步规划见 `PlannerAgent.md`。
- 库存：已纳入闭环（合成库存 + 如实播报 + 加购钳制 + 下单扣减，见 `CartCheckoutAgent.md` 的「库存」节）。

仍可继续扩展：

- Better ranking: 在 `ProductRetriever` 里加入 reranker、用户画像、销量、评分权重（目前为 RRF，无独立 reranker）。
- 真实下单：真实支付、优惠券、物流，以及把会话内的购物车/订单/库存状态持久化到数据库。
